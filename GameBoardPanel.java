...
public class GameBoardPanel extends JPanel implements ActionListener, KeyListener{
	...
	public GameBoardPanel(InetAddress ip2,int port2,InetAddress ip3,int port3,InetAddress ip4,int port4,DatagramSocket clientSocket1,DatagramPacket receivePacket1){
		setFocusable(true);
		addKeyListener(this);
	...
	public void step() throws Exception{
		sendData = ("Client1"+","+paddleOneX +","+ paddleOneY+","+ballX +","+ ballY).getBytes();
		...
		try {
			...
			switch(pad[0]){
			case "Client2":  paddleTwoY = Integer.parseInt(pad[1])-300;
			break;
			case "Client3":  paddleThreeX = 1200-Integer.parseInt(pad[1]);
			break;
			case "Client4":  paddleFourY = 900-Integer.parseInt(pad[1]);
			break;
			}
		} catch (Exception e) {
